...
## Summary
Gasolines prices keep increasing nowadays, we perform an analysis on U.S Gas Prices and try to forecast future gas prices by using Machine Learning Models. Through learning time series models, we found out that ARIMA model could help us to understand the gas prices and predict our future gas prices with higher accuracy. Based on the graph below, we can expect our future gas prices to go down for the rest of the year.
<p align=center>
...
- Contributors
  ...
  - Kimberly Kang: created a mockup of a database with a set of sample data to ensure that the datasets are sourced and are able to be loaded from database. Responsible for dataset updates, download, and ERDs to give a big picture of our dataset. Decided analysis technologies to be used for each step of the project. Built the html, app.py, scraping.py, and Python coding for web scraping of the latest Gas Price news articles. Preprared the PowerPoint structure.
    - Email: kimberlykang0922@gmail.com
